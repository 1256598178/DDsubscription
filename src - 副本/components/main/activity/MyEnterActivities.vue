<template>
	<div class="wrapper">
		...
		<div class="col-nav">
			...
			<text class="col-nav-font" @click="toMyEnterActivitiesNotBegin">未开始</text>
		</div>
		<list class="activity-list">
			...
			<cell class="cell" v-for="(item,index) in actionList" :key="index" v-if="actionList.length > 0">
				...
				<div class="act-list-btn">
					...
					<text class="act-more act-hx" @click="signUp(item.MyActionId)">核销</text>
				</div>
	...
	export default {
		...
		methods:{
			...
			creat(){
				var self = this
				Util.WeexAjax({
				    url: this.ajaxUrl+'?UserId='+self.USERID+'&StatusType=1',
				    method: 'GET',
				    type: 'JSON',
				    token: self.TOKEN,
				    callback: function(ret) {
