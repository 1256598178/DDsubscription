<template>
	<div class="wrapper">
		...
		<div class="headerBox_G">
			<text class="headerBox_G_pageTip">发布定制需求</text>
		...
		<scroller class="formWrapperScroller">
			<div class="formWrapper">
				...
				<div class="messageInfo">
					<div class="labelBox">
						<text class="label">需求描述</text><text class="requiredFlag">*</text>
					</div>
	...
	export default {
		...
		methods: {
			...
			submit(){
				var self = this
				self.temBool.loading = true
				console.log(self.orderForm.money)
				Util.WeexAjax({
					url: self.ajaxUrl,
					method: 'POST',
					type: 'JSON',
			...
			GetOrder() {
				...
				if (this.Id === undefined) {
					this.ajaxUrl = 'api/demand/addMadeDemand'
				} else {
					this.ajaxUrl = 'api/demand/editMadeDemand'
				}
				if(self.orderForm.categoryId != ''){
					if(self.orderForm.phone != ''){
						if((/^1[34578]\d{9}$/.test(self.orderForm.phone))){
							if(self.orderForm.messageInfo.toString().replace(/(^\s+)|(\s+$)/g,"") != ''){
								self.submit()
							}else{
								weex.requireModule("modal").toast({
									message: '留言不能为空，请填写留言',
									duration: 1
								})
							}
						}else{
				...
				weex.requireModule('storage').getItem(self.TOKEN, event => {
					...
					weex.requireModule('storage').getItem(self.lon, event => {
						...
						weex.requireModule('storage').getItem(self.lat, event => {
							...
							if (self.Id != undefined) {
								//console.log(self.Id != undefined);
								Util.WeexAjax({
									url: 'api/demand/getMadeDemand?UserId=' + self.USERID + '&DemandId=' + self.Id,
									method: 'GET',
									type: 'JSON',
									token: self.TOKEN,
									callback: function(ret) {
