	...
	export default {
		...
		methods: {
				...
				Util.WeexAjax({
					...
					callback: function(ret) {
						...
						if (ret.Status == 1) {
							...
							Util.WeexAjax({
								url: 'api/demand/getmydemandlist?Page=' + self.Page + '&PageSize=' + self.PageSize + '&TypeId=' + self.TypeId +
									'&lng=' + self.lng + '&lat=' + self.lat + '&UserId=' + self.USERID,
								method: 'GET',
								type: 'JSON',
								token: self.TOKEN,
								callback: function(ret) {
			...
			onrefresh() {
				...
				self.Page = 1;
				Util.WeexAjax({
					url: self.ajaxUrl + '?Page=' + self.Page + '&PageSize=' + self.PageSize + '&TypeId=' + self.TypeId + '&lng=' +
						self.lng + '&lat=' + self.lat + '&UserId=' + self.USERID,
					method: 'GET',
					type: 'JSON',
					token: self.TOKEN,
					callback: function(ret) {
		...
		mounted() {
			...
			weex.requireModule('storage').getItem(this.USERID, event => {
				...
				weex.requireModule('storage').getItem(this.TOKEN, event => {
					...
					Util.WeexAjax({
						url: self.ajaxUrl + '?Page=' + self.Page + '&PageSize=' + self.PageSize + '&TypeId=' + self.TypeId +
							'&lng=' + self.lon + '&lat=' + self.lat + '&UserId=' + self
							.USERID,
						method: 'GET',
						type: 'JSON',
						token: self.TOKEN,
