	...
	<div class="wrapper">
		...
		<div class="act-nav">
			<div class="act-nav-font act-nav-crt">
				...
				<text class="act-nav-fonts act-nav-red">({{wineLength}})</text>
			</div>
			...
			<cell class="cell" v-for="(item,index) in WineList" :key="index" v-if="WineList.length > 0">
				<div class="act-content">
					...
					<div class="act-stock-b">
						<text class="text-gray shopname">位置：{{item.ShopName}}</text>
						<text class="text-gray nickname">经理人：{{item.NickName}}</text>
						<text class="text-gray">{{item.Expdate}}</text>
					</div>
				</div>
			</cell>
