...
<!--                 <div class="item1">
                ...
                <div class="item2">
                    ...
                    <div class="r">
                       <textarea class="textarea" placeholder="多行输入" v-model="Content"></textarea>
                    </div>
                </div>
            </div>
            <div class="btn">
...
export default {
    ...
    methods:{
        postinfo(){
        var that = this;
        var ajaxUrl='api/orderrecord/addOrderRecordDemand';
            utils.WeexAjax({
                  url: ajaxUrl,
                  method: 'POST',
                  type: 'JSON',
                  ...
                  callback: function(ret) {
                    ...
                    if(ret.Status == 1){
                        that.info = ret;
                        that.infoObj.AddTime = that.AddTime
                        that.infoObj.Content = that.Content
                    }else{
        ...
        close($res=''){
            if($res == 1){
                if(this.Content.length != 0){
                    this.postinfo();
                    this.$emit("transmitObj",this.infoObj)
                }else{
                    weex.requireModule("modal").toast({
                        message: '内容为空,请重新输入',
                        duration: 1
