<template>
	<div class="wrapper">
		...
		<list class="activity-list">
			...
			<cell class="cell" v-for="(item,index) in WithdrawalList" :key="index" @click="jump(item.CashId)" v-if="WithdrawalList.length > 0">
				<div class="act-content">
					<div class="act-stock-t">
						<text class="wine-name">提现-{{item.Account}}</text>
						<text class="wine-num">+{{item.Money}}</text>
					</div>
					<div class="act-stock-b">
						<text class="text-gray">{{item.AddTime}}</text>
						<text class="text-gray text-red" v-if="item.Status == '审核中'">审核中</text>
					</div>
				</div>
			</cell>
			<cell v-if="WithdrawalList.length == 0">
	...
	export default {
		...
		data(){
		    return{
				...
				nomoreName:'暂无记录',
				header_title:'提现记录',
				ajaxUrl:'api/cashwithdrawal/getcashwithdrawallist',
				USERID: 'UserId',
				TOKEN: 'Token',
				Page:1,
				PageSize:10,
			  ...
			  if(self.refreshing) return;
				...
				Util.WeexAjax({
				    ...
				    callback: function(ret) {
				    	...
				    	if(ret.Status == 1){
				    		console.log(rets)
							self.WithdrawalList = rets.obj.Items
						...
						callback: function(ret) {
							...
							if (ret.Status == 1) {
								if (rets.obj.Items.length != 0) {
									self.WithdrawalList = self.WithdrawalList.concat(rets.obj.Items) 
				...
				weex.requireModule('storage').getItem(this.TOKEN,event => {
			        ...
			        Util.WeexAjax({
			            ...
			            callback: function(ret) {
			            	if(ret.Status == 1){
			            		console.log(ret)
			        			self.WithdrawalList = ret.obj.Items
