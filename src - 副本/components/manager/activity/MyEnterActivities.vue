<template>
	<div class="wrapper">
		...
		<div class="act-nav">
			<text class="act-nav-font" @click="newsact">进行中的活动</text>
			<text class="act-nav-font act-nav-crt" @click="myadd">未开始活动</text>
		</div>
<!-- 		<div class="col-nav">
			<text class="col-nav-font col-nav-crt">进行中</text>
			<text class="col-nav-font">未开始</text>
		</div> -->
		<list class="activity-list">
			<cell class="cell" v-for="(item,index) in info" v-if="info.length > 0" :key="index">
				<div class="act-content">
					...
					<div class="act-content-r">
						...
						<div class="act-list-time">
							...
							<text class="act-time">2018年12月1日-2018年12月2日</text>
						</div>
					</div>
				</div>
...
<script>
	...
	export default {
		...
		methods:{
		    ...
		    getinfo(){
			var that = this;
  			var ajaxUrl='api/action/getactionissuelist?UserId='+this.userid+'&StatusType='+0;
				Util.WeexAjax({
			          url: ajaxUrl,
			          method: 'GET',
			          type: 'JSON',
