<template>
    <div class="wrapper" @touchstart="comment(current)">
		...
		<div class="demandTop">
		  ...
		  <image class="addOrderBtn" :src="addOrderBtnImg" style="width:50px;height:50px;" @click="toDemandPageHideFoot('ReleaseFriends')"></image>
		</div>
        <div class="act-nav">
        ...
        <list class="activity-list" @touchstart="comment(current)">
            ...
            <cell class="cell" v-for="(item,index) in recentArr" :key="index" v-if="recentArr.length > 0">
                <div class="act-content">
                    <image class="act-img" :src="item.HeadImageUrl" @click="jump(item.UserId)"></image>
                    <div class="act-content-r">
						...
						<div class="img-list">
							<image :class="['list-img',item.ImageList.length == 1 ? 'f-img':'s-img']" v-for="(val,index) in item.ImageList" :src="val.ImageUrl"></image>
						</div>
                        ...
                        <div class="comments-content">
                            <div class="like-person" v-if="item.CommentList.length > 0">
                                <image class="like-img" :src="likeY"></image>
                                <div v-for="(child,childindex) in item.CommentList" :key="childindex" v-if="child.TypeId == 1">
                                    <text class="text-y" v-if="childindex < 9">{{child.UserName}}</text>
                                ...
                                if (ret.Status == 1) {
                                    ...
                                    utils.WeexAjax({
                                        ...
                                        callback: function(ret) {
                                            console.log(ret)
                                            if(ret.obj.NickName !== null && ret.obj.NickName !== ''){
                                                self.userName = ret.obj.NickName
        ...
        thumbs(msg, index) {
            ...
            utils.WeexAjax({
                ...
                callback: function(ret) {
                    ...
                    if (ret.Status == 1) {
                        console.log(ret)
						self.recentArr[index].CommentList.splice(0,0,{TypeId: 1,UserName: self.userName})
