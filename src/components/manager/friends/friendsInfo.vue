<template>
    <div class="act">
        ...
        <div class="sec1">
            <div class="infobox">
                <div class="imagebox">
                    <image class="infoimg" :src="infoimg" :placeholder="slotimg"></image>
                </div>
                <div class="infotext">
                ...
                <div class="listbox">
                    <div class="item" v-for="(item,index) in newsList" :key="index" v-if="newsList.length > 0">
                        ...
                        <div class="infoimg2box" v-if="item.ImageList.length > 0">
                            <image class="infoimg2" :src="item.ImageList[0].ImageUrl" :placeholder="slotimg"></image>
                        </div>
...
const GETFRINDLIST_URL = 'api/friendcircle/getfriendmessagelist'
...
export default {
            ...
            weex.requireModule('storage').getItem(this.TOKEN, event => {
                ...
                self.getfrind_list({
                    ...
                    Fn: function(ret){
                        ...
                        if(ret.Status == 1){
                            // var items=[]
                            self.newsList=ret.obj.Items
                            for(var i=0;i<self.newsList.length;i++){
                                var timearr=self.newsList[i].Message.AddTime.replace(" ",":").replace(/\:/g,"-").split("-")
                                self.newsList[i].Message.month=timearr[1]
                                self.newsList[i].Message.day=timearr[2]
                            }
                            // self.newsList = items
                        }
    ...
    methods: {
        ...
        getfrind_list(obj){
            var self = this;
            utils.WeexAjax({
                url: GETFRINDLIST_URL + '?Page='+obj.Page+'&PageSize='+self.PageSize+'&ToUserId='+self.ToUserId,
                method: 'GET',
                type: 'JSON',
                token: self.TOKEN,
                callback: function(ret) {
                ...
                Fn: function(ret) {
                    if (ret.obj.Items.length <= 0) {
                        ...
                        for (var item in ret.obj.Items) {
                            self.newsList.push(ret.obj.Items[item])
                        }
                        for(var i=0;i<self.newsList.length;i++){
                            var timearr=self.newsList[i].Message.AddTime.replace(" ",":").replace(/\:/g,"-").split("-")
                            self.newsList[i].Message.month=timearr[1]
